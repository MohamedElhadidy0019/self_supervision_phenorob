...
valcal_dataloader = DataLoader(valcal_dataset, batch_size=128, shuffle=False, 
...
ckpt = torch.load("/data/home/mfarag/farag_data/cp_journal/technical/code/Sugar Beet/DND-SB-main/dataset/benchmark_logs/imagenet/version_17/SwaV/checkpoints/epoch=1199-step=16800.ckpt", map_location="cpu", weights_only=False)
...
def train_epoch(loader):
    model.train()   # set whole model to train mode
    ...
    for x, y in loader:
        ...
        # forward normally (no torch.no_grad)‚
        feats = model[0](x)  
        # for vits make the shape B, C, 1, 1
        # to be acceted by bn layer
        feats = feats.unsqueeze(-1).unsqueeze(-1)             # backbone -> (N, C, 1, 1)
        feats_bn = model[1](feats)        # BatchNorm2d on feature map
        feats_final = torch.flatten(feats_bn, 1)
        logits = model[-1](feats_final)   # head
...
# ==== Loop ====
best_loss = 1e7
os.makedirs("ft_checkpoints", exist_ok=True)
...
for epoch in range(1, epochs + 1):
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        torch.save(model.state_dict(), "/data/home/mfarag/farag_data/cp_journal/technical/code/Sugar Beet/DND-SB-main/dataset/ft_checkpoints/version_17/SwaV/ft_bn_best_val_aug.pt")
